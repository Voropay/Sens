...
import org.sens.core.concept.{Attribute, DataSourceConcept, IntersectConcept, ParentConcept}
...
class IntersectConceptInferenceTests extends AnyFlatSpec with Matchers {

  "Intersect Concept definition" should "infer attributes expressions correctly" in {
    ...
    conDef.inferAttributeExpressions(context).get should equal (IntersectConcept(
      "conceptA",
      ParentConcept(ConceptReference("conceptB"), Some("pcB"), Map(), Nil) ::
        ParentConcept(AnonymousConceptDefinition.builder(
          ...
          ParentConcept(ConceptReference("conceptC"), Some("pcC"), Map(), Nil) :: Nil)
          .attributeDependencies(Equals(ConceptAttribute(List("pcC"), "attr4"), StringLiteral("someValue"))).build,
          None, Map(), Nil) :: Nil,
      Nil
    ))
  }
