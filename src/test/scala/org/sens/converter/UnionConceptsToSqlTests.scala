...
class UnionConceptsToSqlTests extends AnyFlatSpec with Matchers {

  "Intersect Concept definition" should "be converted to SQL correctly" in {
    ...
    conceptSql1 should equal(
      "SELECT `pcB`.`attr1` AS `attr1`, `pcB`.`attr2` AS `attr2`, `pcB`.`attr3` AS `attr3`, `pcB`.`attr4` AS `attr4`\n" +
        "FROM `someFile1` AS `pcB`\n" +
        "INTERSECT\n" +
        "SELECT `c2`.`attr1` AS `attr1`, `c2`.`attr2` AS `attr2`, `c2`.`attr3` AS `attr3`, `c2`.`attr4` AS `attr4`\n" +
        "FROM `someFile2` AS `c2`\n" +
        "WHERE `c2`.`attr5` = 'someType'"
    )
  }

  "Unions Concept definition" should "be converted to SQL correctly" in {
    val context = ValidationContext()
    context.addConcept(ConceptDefinition(DataSourceConcept(
      "conceptB",
      Attribute("attr1", None, Nil) ::
  ...
  "Minus Concept definition" should "be converted to SQL correctly" in {
    val context = ValidationContext()
    context.addConcept(ConceptDefinition(DataSourceConcept(
      "conceptB",
      Attribute("attr1", None, Nil) ::
