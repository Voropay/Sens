...
case class MinusConcept(
  ...
  override def validateAndRemoveVariablePlaceholders(context: ValidationContext): Try[MinusConcept] = {
  ...
  def inferAttributeExpressions(context: ValidationContext): Try[MinusConcept] = {
    Try(MinusConcept(
      name,
