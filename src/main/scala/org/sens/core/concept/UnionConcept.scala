...
case class UnionConcept(
  ...
  override def toSensString: String =
    ...
    "concept " + name + " union of\n" + parentConcepts.map(_.toSensString).mkString(",\n");

  override def validateAndRemoveVariablePlaceholders(context: ValidationContext): Try[UnionConcept] = {
  ...
  def inferAttributeExpressions(context: ValidationContext): Try[UnionConcept] = {
    Try(UnionConcept(
      name,
